...
bool garg_about_to_hammer(Zombie& garg) {
    if (garg.isHammering() && garg.freezeCountdown() <= 0 && garg.fixationCountdown() <= 0) {
        ...
        auto animation_array = AvZ::GetPvzBase()->animationMain()->animationOffset()->animationArray();
...
std::string get_prev_wave_stat(int prev_wave_num) {
    int w = AvZ::GetRunningWave();
    std::ostringstream oss;
    oss << "w" << w << " 至" << AvZ::NowTime(w) << "cs\n";
    for (int i = 1; i < prev_wave_num; i++) {
        w--;
        if (w < 1)
            break;
        oss << "w" << w << " 共" << AvZ::NowTime(w) - AvZ::NowTime(w + 1) << "cs\n";
    }
    return oss.str();
}
...
class overwatch : public AvZ::TickRunner {
...
private:
    ...
    int check_enter_home(Zombie& z) {
        ...
        if (zombie_about_to_enter_home(z)) {
            ...
            if (stop_when_zombie_enter_home) {
                AvZ::ShowErrorNotInQueue("僵尸进家: " + zombie_info(z).str());
                stop_run();
                return 1;
            }
            z.state() = 3;
    ...
    void check_garg(Zombie& garg, std::set<AvZ::Grid>& grids_about_to_be_smashed) {
        using namespace ow_priv;
        if (garg_about_to_hammer(garg)) {
            ...
            for (auto& p : AvZ::alive_plant_filter) {
                if (garg.row() == p.row() && zombie_attack_rect.get_rect_overlap(get_plant_rect(p)) >= 20
                    ...
                    if (watched_plants.count(grid_with_type(p))) {
                        smash_stats[grid_with_type(p)].add(AvZ::GetMainObject()->wave() - 1, 1);
                        garg.state() = 3;
                    } else
    ...
    void restore_health() {
        for (auto& p : AvZ::alive_plant_filter) {
            if (watched_plants.count(grid_with_type(p)) && p.hp() && p.hp() < INIT_HP) {
                hp_stats[grid_with_type(p)].add(AvZ::GetMainObject()->wave() - 2, INIT_HP - p.hp());
                p.hp() = INIT_HP;
            }
        }
    }
    ...
    void stop_run() {
        using namespace ow_priv;
        AvZ::ShowErrorNotInQueue(get_prev_wave_stat(3));
        running_state = RunningState::ShuttingDown;
        shutting_down_time = AvZ::NowTime(0);
    }

    void run() {
        using namespace ow_priv;
        if (running_state != RunningState::Running) {
            if (running_state == RunningState::ShuttingDown && AvZ::NowTime(0) >= shutting_down_time + 2) {
                running_state = RunningState::Done;
                AvZ::SetAdvancedPause(true);
            }
            return;
        }

        std::set<AvZ::Grid> grids_about_to_be_smashed;
        ...
        for (auto& p : AvZ::alive_plant_filter) {
            if (safe_plants.count(grid_with_type(p)) && p.hp() != p.hpMax()) {
                AvZ::ShowErrorNotInQueue("#-# 安全位受损, hp=#", p.row() + 1, p.col() + 1, p.hp());
                stop_run();
                return;
            }
        }
        if (AvZ::NowTime(AvZ::GetMainObject()->wave()) == 0)
            restore_health();
    }
...
public:
    ...
    void start(const std::set<AvZ::Grid>& watched_grids, const std::set<AvZ::Grid>& safe_grids = {},
        bool stop_when_zombie_enter_home = false) {
        using namespace ow_priv;
        ...
        AvZ::InsertOperation([this] { pushFunc([this] { run(); }); }, "Overwatch::start");
    }
    ...
    void dump_stats() {
        ...
        oss << "注：巨人砸炮按砸炮波数显示\n";
        AvZ::ShowErrorNotInQueue(oss.str());
    }
};