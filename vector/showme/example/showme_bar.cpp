 ...
 * @LastEditors: qrmd
 * @LastEditTime: 2022-10-07 17:44:58
...
int hp_bar_height = 15; // 水平生命条的高度（像素）
    ...
    for (int index = 0; index < GetMainObject()->zombieTotal(); ++index) {
        if (!zombies[index].isDisappeared() && !zombies[index].isDead()) {
            ...
            if (zombies[index].type() == POLE_VAULTING_ZOMBIE && zombies[index].state() == 12) {
                ...
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00), SMPainter::RIGHT, circulation_rate, {});
            }
            // 读报僵尸失去报纸
            ...
            if (zombies[index].type() == BUNGEE_ZOMBIE && (zombies[index].state() == 6 || zombies[index].state() == 7)) {
                ...
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00), SMPainter::RIGHT, circulation_rate, {300.0 / (300 + 63)});
            }
            // 投篮僵尸投篮
            if (zombies[index].type() == CATAPULT_ZOMBIE && (zombies[index].state() == 67 || zombies[index].state() == 68)) {
                ...
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00), SMPainter::RIGHT, circulation_rate, {SHOOT_BASKETBALL_CIRCULATION_RATE});
            }
            // 巨人僵尸锤击
            if ((zombies[index].type() == GIGA_GARGANTUAR || zombies[index].type() == GARGANTUAR) && zombies[index].isHammering()) {
                ...
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00), SMPainter::RIGHT, circulation_rate, {HAMMER_CIRCULATION_RATE});
            }
            // 巨人僵尸投掷小鬼
            if ((zombies[index].type() == GIGA_GARGANTUAR || zombies[index].type() == GARGANTUAR) && zombies[index].state() == 69) {
                ...
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00), SMPainter::RIGHT, circulation_rate, {THROW_IMP_CIRCULATION_RATE});
            }
            // 僵尸冰冻倒计时
            ...
            if (zombies[index].freezeCountdown() > 0) {
                ...
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0x0E, 0xA0, 0xE0), SMPainter::RIGHT, circulation_rate, {});
            }
            ...
            if (zombies[index].type() == GARGANTUAR) {
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0x00, 0xFF, 0x00), SMPainter::RIGHT, circulation_rate, {1800 / 3000.0});
            } else if (zombies[index].type() == GIGA_GARGANTUAR) {
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0x00, 0xFF, 0x00), SMPainter::RIGHT, circulation_rate, {1800 / 6000.0, 1800 * 2 / 6000.0, 1800 * 3 / 6000.0});
            } else {
                bar_painter.drawBar(bar_pos_x, bar_pos_y, bar_width, bar_height, 1, SMArgb(0xFF, 0x00, 0xFF, 0x00), SMPainter::RIGHT, circulation_rate, {});
            }
        }
    }
        ...
        if (!plants[index].isDisappeared() && !plants[index].isCrushed()) {
            ...
            if (plants[index].type() == CHERRY_BOMB || plants[index].type() == ICE_SHROOM || plants[index].type() == DOOM_SHROOM || plants[index].type() == JALAPENO) {
                if (plants[index].explodeCountdown() > 0) {
                    ...
                    bar_painter.drawBar(bar_pos_x, bar_pos_y, 72, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00), SMPainter::RIGHT, circulation_rate, {});
                }
            }
            ...
            if (plants[index].type() == BLOVER) {
                if (plants[index].bloverCountdown() > 0) {
                    ...
                    bar_painter.drawBar(bar_pos_x, bar_pos_y, 72, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00), SMPainter::RIGHT, circulation_rate, {(250 - 50) / 250.0});
                }
            }
            // 磁力菇复用进度条
            ...
            if (plants[index].type() == COB_CANNON && plants[index].state() != 37) {
                ...
                bar_painter.drawBar(bar_pos_x + 40, bar_pos_y, 2 * 72, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00), SMPainter::RIGHT, circulation_rate, {});
            }
            // 植物血量
            bar_pos_y = plants[index].yi();
            circulation_rate = plants[index].hp() / float(plants[index].hpMax());
            if (plants[index].type() == COB_CANNON) {
                bar_painter.drawBar(bar_pos_x + 40, bar_pos_y, 2 * 72, bar_height, 1, SMArgb(0xFF, 0x00, 0xFF, 0x00), SMPainter::RIGHT, circulation_rate, {});
            } else {
                bar_painter.drawBar(bar_pos_x, bar_pos_y, 72, bar_height, 1, SMArgb(0xFF, 0x00, 0xFF, 0x00), SMPainter::RIGHT, circulation_rate, {});
            }
        }
    }
    int x_diff = 80;                                                                          // 列间距
