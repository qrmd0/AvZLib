 ...
 * @LastEditTime: 2022-11-25 22:42:10
...
float SHOOT_BASKETBALL_CIRCULATION_RATE =
    0.243; // 投篮僵尸抛出篮球时的动画循环率
...
void Draw() {
  ...
  auto animations = (AnimationAdd *)GetPvzBase()
                        ->animationMain()
                        ->animationOffset()
                        ->animationArray();
  ...
  for (int index = 0; index < GetMainObject()->zombieTotal(); ++index) {
    if (!zombies[index].isDisappeared() && !zombies[index].isDead()) {
      bar_pos_x =
          int((2 * zombies[index].abscissa() + zombies[index].bulletAbscissa() +
               zombies[index].hurtWidth()) /
              2);
      // 僵尸动画进度
      bar_pos_y =
          int(zombies[index].ordinate() + zombies[index].bulletOrdinate() -
              (bar_height * 3 + 5 * frame_size));
      // 撑杆僵尸跳跃
      if (zombies[index].type() == POLE_VAULTING_ZOMBIE &&
          zombies[index].state() == 12) {
        ...
        bar_painter.drawBar(bar_pos_x, bar_pos_y, circulation_rate, bar_width,
                            bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00),
                            SMPainter::RIGHT, {});
      }
      // 读报僵尸失去报纸
      if (zombies[index].type() == NEWSPAPER_ZOMBIE &&
          zombies[index].state() == 30) {
        index_animation = zombies[index].animationCode();
        circulation_rate = animations[index_animation].circulationRate();
      ...
      // // 舞王僵尸和伴舞僵尸跳舞
      // if ((zombies[index].type() == DANCING_ZOMBIE || zombies[index].type()
      ...
      //     bar_width, bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00),
      //     SMPainter::RIGHT, {});
      // }
      // 潜水僵尸入水
      if (zombies[index].type() == SNORKEL_ZOMBIE &&
          zombies[index].state() == 58) {
        index_animation = zombies[index].animationCode();
        circulation_rate = animations[index_animation].circulationRate();
      ...
      // 海豚僵尸入水和跳跃
      if (zombies[index].type() == DOLPHIN_RIDER_ZOMBIE &&
          (zombies[index].state() == 52 || zombies[index].state() == 54)) {
        index_animation = zombies[index].animationCode();
        circulation_rate = animations[index_animation].circulationRate();
      ...
      // 玩偶匣僵尸开盒
      if (zombies[index].type() == JACK_IN_THE_BOX_ZOMBIE &&
          zombies[index].state() == 16) {
        circulation_rate = 1 - zombies[index].stateCountdown() /
                                   float(JACK_IN_THE_BOX_OPEN_BOX_TIME);
      ...
      // 气球僵尸落地
      if (zombies[index].type() == BALLOON_ZOMBIE &&
          zombies[index].state() == 74) {
        index_animation = zombies[index].animationCode();
        circulation_rate = animations[index_animation].circulationRate();
      ...
      // 矿工僵尸出土后眩晕
      if (zombies[index].type() == DIGGER_ZOMBIE &&
          zombies[index].state() == 36) {
        index_animation = zombies[index].animationCode();
        circulation_rate = (animations[index_animation].circulationRate() +
                            animations[index_animation].circulationCount()) /
                           2.0;
      ...
      // 蹦极僵尸停留
      if (zombies[index].type() == BUNGEE_ZOMBIE &&
          (zombies[index].state() == 6 || zombies[index].state() == 7)) {
        if (zombies[index].state() == 6) {
          circulation_rate =
              1 - (zombies[index].stateCountdown() + BUNGEE_GRAB_TIME) /
                      float(BUNGEE_REMAIN_TIME + BUNGEE_GRAB_TIME);
        } else if (zombies[index].state() == 7) {
          index_animation = zombies[index].animationCode();
          circulation_rate = (BUNGEE_REMAIN_TIME +
                              animations[index_animation].circulationRate() *
                                  BUNGEE_GRAB_TIME) /
                             float(BUNGEE_REMAIN_TIME + BUNGEE_GRAB_TIME);
        }
      ...
      // 投篮僵尸投篮
      if (zombies[index].type() == CATAPULT_ZOMBIE &&
          (zombies[index].state() == 67 || zombies[index].state() == 68)) {
        circulation_rate = 1 - zombies[index].stateCountdown() /
                                   float(SHOOT_BASKETBALL_COUNTDOWN);
      ...
      if ((zombies[index].type() == GIGA_GARGANTUAR ||
           zombies[index].type() == GARGANTUAR) &&
          zombies[index].isHammering()) {
        index_animation = zombies[index].animationCode();
        circulation_rate = animations[index_animation].circulationRate();
      ...
      bar_pos_y =
          int(zombies[index].ordinate() + zombies[index].bulletOrdinate() -
              (2 * bar_height + 4 * frame_size));
      freeze_countdown = zombies[index].freezeCountdown();
      if (zombies[index].freezeCountdown() > 0) {
        circulation_rate =
            zombies[index].freezeCountdown() / float(FREEZE_TIME);

        bar_painter.drawBar(bar_pos_x, bar_pos_y, circulation_rate, bar_width,
                            bar_height, 1, SMArgb(0xFF, 0x0E, 0xA0, 0xE0),
                            SMPainter::RIGHT, {});
      }
      ...
      bar_pos_y =
          int(zombies[index].ordinate() + zombies[index].bulletOrdinate() -
              (bar_height + 2 * frame_size));
      circulation_rate =
          (zombies[index].hp() + zombies[index].oneHp()) /
          float(zombies[index].hpMax() + zombies[index].oneHpMax());
      if (zombies[index].type() == GARGANTUAR) {
        bar_painter.drawBar(bar_pos_x, bar_pos_y, circulation_rate, bar_width,
                            bar_height, 1, SMArgb(0xFF, 0x00, 0xFF, 0x00),
        ...
        bar_painter.drawBar(
            ...
            SMArgb(0xFF, 0x00, 0xFF, 0x00), SMPainter::RIGHT,
  ...
  for (int index = 0; index < GetMainObject()->plantTotal(); ++index) {
    ...
    if (!plants[index].isDisappeared() && !plants[index].isCrushed()) {
      ...
      if (plants[index].type() == CHERRY_BOMB ||
          plants[index].type() == ICE_SHROOM ||
          plants[index].type() == DOOM_SHROOM ||
          plants[index].type() == JALAPENO) {
      ...
      if (plants[index].type() == COFFEE_BEAN) {
        if (plants[index].explodeCountdown() > 0) {
          circulation_rate = (plants[index].explodeCountdown() +
                              plants[index].bloverCountdown()) /
                             299.0;
      ...
      if (plants[index].type() == SUNFLOWER ||
          plants[index].type() == TWIN_SUNFLOWER ||
          plants[index].type() == SUN_SHROOM ||
          plants[index].type() == MARIGOLD) {
      ...
      if (plants[index].type() == SQUASH) {
        if (plants[index].state() >= 3 && plants[index].state() <= 6) {
          if (plants[index].state() == 3) {
            circulation_rate = (plants[index].stateCountdown() + 45 + 50 + 7) /
                               float(80 + 45 + 50 + 7);
          } else if (plants[index].state() == 4) {
            circulation_rate = (plants[index].stateCountdown() + 50 + 7) /
                               float(80 + 45 + 50 + 7);
          } else if (plants[index].state() == 5) {
            circulation_rate =
                (plants[index].stateCountdown() + 7) / float(80 + 45 + 50 + 7);
          } else if (plants[index].state() == 6) {
            circulation_rate =
                (plants[index].stateCountdown()) / float(80 + 45 + 50 + 7);
          }
          bar_painter.drawBar(bar_pos_x, bar_pos_y, circulation_rate, 72,
                              bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00),
                              SMPainter::RIGHT, {});
      ...
      if (plants[index].type() == COB_CANNON && plants[index].state() != 37) {
        ...
        case 35:
          circulation_rate = (125 + plants[index].stateCountdown()) /
                             float(CANNON_LOADING_TIME);
          break;
        case 36:
          circulation_rate =
              (int(125 * (1 - animations[index_animation].circulationRate()) +
                   0.5) +
               1) /
              float(CANNON_LOADING_TIME);
          break;
        case 38:
          circulation_rate =
              (3125 +
               int(350 * (1 - animations[index_animation].circulationRate()) +
                   0.5)) /
              float(CANNON_LOADING_TIME);
          break;
        default:
      ...
      if (plants[index].type() == COB_CANNON) {
        ...
        if (plants[index].type() == LILY_PAD ||
            plants[index].type() == FLOWER_POT) {
          bar_pos_y = bar_pos_y =
              plants[index].yi() + (2 * bar_height + 4 * frame_size);
        } else if (plants[index].type() == PUMPKIN ||
                   plants[index].type() == COFFEE_BEAN) {
          bar_pos_y = bar_pos_y =
              plants[index].yi() + (4 * bar_height + 8 * frame_size);
        }
        bar_painter.drawBar(bar_pos_x, bar_pos_y, circulation_rate, 72,
                            bar_height, 1, SMArgb(0xFF, 0x00, 0xFF, 0x00),
                            SMPainter::RIGHT, {});
  ...
  int x_diff = 80; // 列间距
  int y_diff = (GetMainObject()->scene() == 2 || GetMainObject()->scene() == 3)
                   ? 85
                   : 100; // 行间距
  // 场地物品进度条
  for (int index = 0; index < main_object_add->groundItemTotal(); ++index) {
    if (!groundItems[index].isDisappeared()) {
      if (groundItems[index].type() == 2) {
        ...
        circulation_rate =
            groundItems[index].countdown() / CRATER_REMAINING_TIME;
        bar_painter.drawBar(bar_pos_x, bar_pos_y, circulation_rate, 72,
                            bar_height, 1, SMArgb(0xFF, 0xFF, 0xC0, 0x00),
                            SMPainter::RIGHT, {});
      }
