...
float GetZombieAbscissa(int index_zombie, int time_length)
{
    ...
    if (is_uniform) {                 // 对于匀速移动模型
        ...
        for (int i = 0; i < fragment_number; ++i) {
            ...
            whole_movement += time_in_fragments[i] * (frame_abscissas[i + 1] - frame_abscissas[i]);
        }
        whole_movement = whole_movement * zombie_speed * 47 * (fragment_number + 1) * 0.01 / total_movement;
        // 计算最终结果
