...
// ***Not In Quene
// 返回[index_zombie]号僵尸保持当前运动模型（未再次受到减速、冰冻、定身等限制移动效果、进入啃食状态、鸭子僵尸入水、撑杆僵尸遇到植物跳跃等）移动时，未来[time_length]厘秒内的横坐标向量，如果该僵尸是无法预测的类型或处于无法预测的状态，则返回空向量
...
// abscissas = GetZombieAbscissas(10, 300);
// if (!abscissas.empty()) {
//     for (int time = 0; time < abscissas.size(); ++time) {
//         ShowErrorNotInQueue("#号僵尸在#厘秒后的横坐标为：#", 10, time, abscissas[time]);
//     }
// } else {
//     ShowErrorNotInQueue("#号僵尸是无法预测的类型或处于无法预测的状态", 10);
// }
