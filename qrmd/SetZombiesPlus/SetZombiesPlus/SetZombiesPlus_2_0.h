 ...
 * @LastEditors: qrmd
 * @LastEditTime: 2023-07-31 13:39:44
 * @Description:SetZomies的增强版本，更自由地设置出怪
 * 使用方法：1、在VsCode中运行命令AvZ:Get AvZ Extension，在列表中选择本插件；
 *          2、在{AvZ安装目录}/inc文件夹中找到本插件，浏览README和代码注释了解调用方法；
 *          3、在要应用本插件的脚本开头加上 #include "SetZombiesPlus/main.h"
...
ALogger<AMsgBox> _tempMsgBoxLogger;
...
// 此行代码必须编写于对应的ASetZombiesLimits前以生效
void ASetWaitLimit(int time);

// *** Not In Queue
// 设置是否显示为了满足限定条件刷新出怪列表的调试信息，默认为否
// 此行代码必须编写于对应的ASetZombiesLimits前以生效
void ASetIsShowInfo(bool is_true);
...
void ASetZombiesPlus(std::vector<int> types, SetZombiesMethod method = INTERNAL);
...
// ASetZombiesLimits({{9, GIGA_GARGANTUAR, "==", 0}, {10, ZOMBIE, ">=", 10}, {TRANS_WAVE, GIGA_GARGANTUAR, ">=", 16}});
void ASetZombiesLimits(std::vector<ZombiesRefreshLimit> zombies_limits);

void ASetZombiesLimits(ZombiesRefreshLimit each);

void ASetZombiesLimits(int wave, int type, std::string method, int number);
...
// ASetWaveZombieList(18, {{IMP, 20}, {ABUNGEE_ZOMBIE, 14}, {GARGANTUAR, 16}});
void ASetWaveZombieList(int wave, std::vector<WaveZombies> wave_zombies);

} // namespace _qrmd

using _qrmd::ASetIsShowInfo;
using _qrmd::ASetWaitLimit;
using _qrmd::ASetWaveZombieList;
using _qrmd::ASetZombiesLimits;
using _qrmd::ASetZombiesPlus;
using _qrmd::AVERAGE;
...
void _qrmd::SetZombiesAverage(std::vector<int> types)
{
    ...
    const std::vector<int> ban_zombie_types = {AFLAG_ZOMBIE, ABUNGEE_ZOMBIE};
    ...
    auto zombie_list = AGetMainObject()->ZombieList();
    for (int index = 0; index < 1000; ++index, ++zombie_list) {
        (*zombie_list) = zombie_types[index % zombie_types.size()];
    }

    // 生成旗帜僵尸
    if (std::find(types.begin(), types.end(), AFLAG_ZOMBIE) != types.end()) {
        for (auto index : {450, 950}) {
            (*(AGetMainObject()->ZombieList() + index)) = AFLAG_ZOMBIE;
        }
    }

    // 生成蹦极僵尸
    /*if (std::find(types.begin(), types.end(), ABUNGEE_ZOMBIE) != types.end()) {
        for (auto index : {451, 452, 453, 454, 951, 952, 953, 954}) {
            (*(AGetMainObject()->ZombieList() + index)) = ABUNGEE_ZOMBIE;
        }
    }*/
...
void _qrmd::SetZombiesInternal(std::vector<int> types)
{
    ...
    for (int i = AZOMBIE; i <= AGIGA_GARGANTUAR; ++i) {
...
void _qrmd::ASetZombiesPlus(std::vector<int> types, SetZombiesMethod method)
{

    const std::vector<int> ban_zombie_types = {ABACKUP_DANCER, AZOMBIE_BOBSLED_TEAM, AZOMBIE_YETI, AIMP, ADR_ZOMBOSS};
    std::vector<int> zombie_types = {};
    ...
    if (zombie_types.empty()) {
        ALogger<AMsgBox> _tempMsgBoxLogger;
        _tempMsgBoxLogger.Error("ASetZombiesPlus的有效僵尸种类数量不得为0");
        return;
    }
    ...
    case INTERNAL:
        if (types.size() < 2) {
            ALogger<AMsgBox> _tempMsgBoxLogger;
            _tempMsgBoxLogger.Error("ASetZombiesPlus以自然方式出怪时的有效僵尸种类数量不得小于2，如果您想仅刷出一种僵尸，请使用极限方式出怪");
            return;
        } else {
...
void _qrmd::ASetIsShowInfo(bool is_true)
{
...
void _qrmd::ASetZombiesLimits(std::vector<ZombiesRefreshLimit> zombies_limits)
{
    bool is_succeed;
    int zombie_number;
    int trans_wave;
    ...
    for (int count = 0;; ++count) {
        ...
        for (ZombiesRefreshLimit each : zombies_limits) {
            if (!AGetZombieTypeList()[each.type]) {
                ALogger<AMsgBox> _tempMsgBoxLogger;
                _tempMsgBoxLogger.Error("ASetZombiesLimits限定的僵尸种类必须属于本轮出怪种类");
                return;
            }
            if (each.wave == TRANS_WAVE) {
                if (each.type != AGIGA_GARGANTUAR) {
                    ALogger<AMsgBox> _tempMsgBoxLogger;
                    _tempMsgBoxLogger.Error("ASetZombiesLimits的限定条件为变速时，第二个参数必须为GIGA_GARGANTUAR或其别名");
                    return;
                }
                trans_wave = 22;
                ...
                ALogger<AMsgBox> _tempMsgBoxLogger;
                _tempMsgBoxLogger.Error("ASetZombiesLimits的限定条件的第一个参数必须属于[1, 20]或为TRANS/TOTAL_WAVE");
                return;
            }
        }
        if (is_succeed) {
...
void _qrmd::ASetZombiesLimits(ZombiesRefreshLimit each)
{
...
void _qrmd::ShowInfo(bool is_successful, int count, int time)
{
    if (is_successful) {
        ALogger<AMsgBox> _tempMsgBoxLogger;
        _tempMsgBoxLogger.Info("找到了符合限定条件的出怪列表\n刷新次数：#\n耗时：#秒", count, time / 1000.0);
    } else {
        ALogger<AMsgBox> _tempMsgBoxLogger;
        _tempMsgBoxLogger.Error("未能找到符合限定条件的出怪列表，您可以延长等待时间或放宽限定条件重试\n刷新次数：#\n耗时：#秒", count, time / 1000.0);
    }
}
...
void _qrmd::ASetWaveZombieList(int wave, std::vector<WaveZombies> wave_zombies)
{
    int number = 0;
    auto zombie_list = AGetMainObject()->ZombieList();
    ...
    if (number != 50) {
        ALogger<AMsgBox> _tempMsgBoxLogger;
        _tempMsgBoxLogger.Error("ASetWaveZombieList设置的一波僵尸的数量必须为50，而当前的数量为#", number);
        return;
    }
    int index = (wave - 1) * 50;
    for (WaveZombies each : wave_zombies) {
